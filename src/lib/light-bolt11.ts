...
function featureBitsEncoder(featureBits: FeatureBits) {
  ...
  return new Array(wordsLength)
    ...
    .map(
      (_, index) =>
        (((bools[index * 5 + 4] ?? 0) as unknown as number) << 4) |
        (((bools[index * 5 + 3] ?? 0) as unknown as number) << 3) |
        (((bools[index * 5 + 2] ?? 0) as unknown as number) << 2) |
        (((bools[index * 5 + 1] ?? 0) as unknown as number) << 1) |
        (((bools[index * 5] ?? 0) as unknown as number) << 0),
    )
    .reverse();
}
