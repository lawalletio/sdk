import { NDKSigner, NDKUser, type NostrEvent } from '@nostr-dev-kit/ndk';
import crypto from 'crypto';
import { nip04 } from 'nostr-tools';
import { nowInSeconds, parseContent } from './utils.js';
...
export async function extendedEncrypt(signer: NDKSigner, receiverPubkey: string, message: string): Promise<string> {
  if (!signer) return '';

  try {
    const user = new NDKUser({ pubkey: receiverPubkey });
    const encryptedMessage = await signer.encrypt(user, message);
...
export async function extendedDecrypt(
  signer: NDKSigner,
  senderPubkey: string,
  ...
  try {
    ...
    const decryptedMessage = await signer.decrypt(user, encryptedMessage);
    return decryptedMessage;
  } catch {
    return '';
  }
